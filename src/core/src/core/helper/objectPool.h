...
namespace Cjing3D
{
	namespace ObjectPoolImpl
	{
		...
		{
		private:
			static const U32 MIN_BLOCK_ALIGN = 64;
		...
		public:
			...
			static ObjectPoolBlock<T>* Create(size_t blockSize)
			{
				...
				ObjectPoolBlock<T>* ptr = reinterpret_cast<ObjectPoolBlock<T>*>(CJING_MALLOC_ALIGN(totalSize, MIN_BLOCK_ALIGN));
				if (ptr != nullptr)
				{
					new(ptr)ObjectPoolBlock<T>(blockSize);
	...
	private:
		...
		BlockInfo* AllocateBlock()
		{
			...
			if (mBlockInfos == nullptr)
			{
				mBlockInfos = reinterpret_cast<BlockInfo*>(CJING_MALLOC(mBlockCount * sizeof(BlockInfo)));
			}
			else
			{
				mBlockInfos = reinterpret_cast<BlockInfo*>(CJING_REMALLOC(mBlockInfos, mBlockCount * sizeof(BlockInfo)));
			}
