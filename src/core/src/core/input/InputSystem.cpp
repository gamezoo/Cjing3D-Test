...
namespace Cjing3D
{
	...
	void InputManager::Update(F32 deltaTime)
	{
		...
		{
			KeyCode keyCode = it.key().mKeyCode;
			U32 index = it.key().mControllerIndex;
			U32 inputSecond = it.value();

			bool toRemoved = false;
			if (IsKeyDown(keyCode, index)) {
				it.value() = inputSecond + 1;
			}
			else {
				toRemoved = true;
			}
	...
	bool InputManager::IsKeyReleased(const KeyCode& key, U32 index)
	{
		...
		auto it = mInputKeys.find(inst);
		if (it != mInputKeys.end() && it.value() > 0) {
			return true;
		}
		else
		{
	...
	bool InputManager::IsKeyHold(const KeyCode& key, U32 index, U32 frames, bool continuous)
	{
		...
		else if ((!continuous && it.value() == frames) ||
				  (continuous && it.value() >= frames))
		{
			return true;
		}
		else
